...
async function test() {
  ...
  stream.on("data", async(data) => {
    ...
    if (data.transaction && data.transaction.transaction ){
        ...
        // const {virtualsolreserve,virtualtokenreserve} = calculateVirtualReserve(bond_info!.creator_reserve,bond_info!.total_supply)
        console.log(bond_info!.bonding_curve,bond_info!.mint,0.125,0.15,0.00,0.002,bond_info!.associated_bonding_curve,virtualtokenreserve,virtualsolreserve,block_hash,bond_info!.total_supply)
        // const amount 
